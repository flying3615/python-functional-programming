...
class Order(Immutable):
    ...
    def __init__(self, orderid, shipping_address, expedited, shipped, customer, order_items):
        ...
        notint = lambda x: not isinstance(x, int)
        notbool = lambda x: not isinstance(x, bool)
        notcust = lambda x: not isinstance(x, Customer)
        notstr = lambda x: not isinstance(x, str)
        notiter = lambda x: not isinstance(x, Iterable)
        notitems = lambda x: not all(isinstance(i, OrderItem) for i in x)

        with Match((orderid, shipping_address, customer)) as m:
            m((notint, ..., ...))  >> error('Invalid order id.')
            m((..., notstr, ...))  >> error('Invalid shipping address.')
            m((..., '', ...))      >> error('Invalid shipping address.')
            m((..., ..., notcust)) >> error('Invalid customer reference.')
            m(...)                 >> (lambda x:None)

        with Match((expedited, shipped, order_items)) as m:
            m((notbool, ..., ...))  >> error('Invalid expedited indicator.')
            m((..., notbool, ...))  >> error('Invalid shipped indicator.')
            m((..., ..., notiter))  >> error('Invalid order items collection. Not iterable.')
            m((..., ..., notitems)) >> error('Invalid order items collection. Not all order items.')
            m(...)                  >> (lambda x:None)
